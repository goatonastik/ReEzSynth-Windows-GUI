...
2. Leave the large dependency-install option unchecked on the finish page.
...
Follow `INSTALL_WINDOWS.md` and use the Start-menu dependency setup shortcut. The
setup requires Git, Conda, the CUDA 12.8 toolkit, and Visual Studio 2022 C++ tools
for the supported NVIDIA path. It can download several gigabytes.
...
4. Uninstall ReEzSynth. Confirm that application files and shortcuts are removed,
   while the external project and separately created Conda environments remain.
