...
__global__ void corr_backward_kernel(
  ...
  if (idx < B*H1*W1) {
    ...
    const int N = coords.size(1);
    const int C = fmap1.size(3);
    const int rd = int(sqrtf(static_cast<float>(corr_grad.size(2))));
...
std::vector<torch::Tensor> corr_cuda_forward(
  ...
  int radius)
{
  const c10::cuda::CUDAGuard guard(fmap1.device());
  ...
  corr_forward_kernel<float><<<blocks, threads, 0, c10::cuda::getCurrentCUDAStream()>>>(
