...
def main():
    ...
    runtime = prepare_runtime({'engine': FUOUM}, {'fuoum_python': sys.executable}, require_native=False)
    compatibility = ROOT / 'patches' / 'fuoum-windows-torch-types.patch'
    applied = subprocess.run(['git', 'apply', '--reverse', '--check', str(compatibility)],
                             cwd=runtime['source'], capture_output=True)
    if applied.returncode:
        subprocess.run(['git', 'apply', '--check', str(compatibility)], cwd=runtime['source'], check=True)
        subprocess.run(['git', 'apply', str(compatibility)], cwd=runtime['source'], check=True)
    import torch
