...
def run_bidirectional_pass(pipeline, seq, style_img, is_forward, content_frames):
    ...
    images, errors, flows, nnfs = array_sequence([style_img]), array_sequence(), array_sequence(), array_sequence()
    previous_nnf = None
...
def run_compatible_pass(pipeline, seq, style_img, is_forward, content_frames):
    """Pinned FuouM pass semantics with bounded frame/error/NNF storage."""
    from ezsynth.utils.warp_utils import Warp, PositionalGuide
    h, w = content_frames[0].shape[:2]
    key = seq.start_frame if is_forward else seq.end_frame
    step = 1 if is_forward else -1
    end = seq.end_frame if is_forward else seq.start_frame
    warp = Warp(h, w)
    source_pos = PositionalGuide(h, w).get_pristine_guide_uint8()
    images, errors, flows, nnfs = array_sequence([style_img]), array_sequence(), array_sequence(), array_sequence()
    previous_nnf = None
    propagate = pipeline.config.pipeline.use_temporal_nnf_propagation
    for source in range(key, end, step):
        target = source + step
        flow = pipeline._fwd_flows[source if is_forward else target]
        guides = pipeline._prepare_guides_for_frame(
            keyframe_idx=key, target_idx=target, style_img=style_img,
            warped_previous_style=warp.run_warping(images[-1], flow * (-step)),
            source_pos_guide=source_pos, target_pos_guide=PositionalGuide(h, w).create_from_flow(flow),
            content_frames=content_frames)
        initial = (None if previous_nnf is None else
                   warp.run_warping(previous_nnf.astype(np.float32), flow * (-step)).astype(np.int32))
        result = pipeline.synthesis_engine.run(style_img, guides=guides,
            initial_nnf=initial, output_nnf=propagate)
        images.append(result[0])
        errors.append(result[1])
