...
| --- | --- | --- |
| Six synthesis parameters | Exposed | Audit numeric domains, including automatic pyramid depth (-1), which is currently rejected |
| Four guide weights | Exposed | Audit arbitrary frontend maximums; preserve defaults |
...
- cupy, timm and yaml are not discoverable in the current interpreter. Metadata
  checks also found no cupy, cupy-cuda12x, timm or PyYAML distributions.
